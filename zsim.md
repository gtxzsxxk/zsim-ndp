...
AggregateStat 是 statistics infrastructure 中的 Topological Building Block。它实现了 Composite Design Pattern，在 zsim 的 statistics architecture 中扮演 Structural Container 的角色。

zsim 的 statistics 数据被组织成一个 DAG (Directed Acyclic Graph) 或 Tree structure。

- Leaf Nodes：如 Counter, ScalarStat, VectorStat，它们负责实际存储 numerical data。
- Internal Nodes：即 AggregateStat。它们不存储 value，而是维护一个指向其他 Stat 对象的指针列表 (`g_vector<Stat*> _children`)。

#### 层级化组织 (Hierarchical Organization)

它允许模拟器将逻辑上相关的统计数据分组。

- 封装 (Encapsulation)：例如，一个 Cache 对象会创建一个 AggregateStat，并将所有与该缓存相关的计数器（Hits, Misses）append 到这个 Aggregate 中。
- 命名空间 (Namespacing)：通过层层嵌套，形成如 system.core0.l1d.hits 的路径结构。这使得后端（Backend）在输出数据时能生成结构化的日志（如缩进的文本或 HDF5 的 Group）。

#### 生命周期管理 (Lifecycle Management)

AggregateStat 强制执行严格的可变性阶段 (Mutability Phases)，以确保统计结构的一致性：

1. 初始化阶段 (Mutable Phase):
    - 在模拟器启动 (Init) 期间，_isMutable 为 true。
    - 各组件调用 init() 设置名称，并调用 append() 动态构建树结构。
2. 冻结阶段 (Finalization Phase):
    - 在模拟开始前，调用 makeImmutable()。
    - 剪枝 (Pruning)：它会递归地检查子节点。如果一个 AggregateStat 不包含任何有效的数据节点（即它是空的），它会被从树中移除。这防止了输出文件中出现空的 Group。
3. 输出阶段 (Immutable Phase): 在模拟结束 (SimEnd) 或周期性转储时，后端访问该对象。此时结构已锁定，保证了多线程环境下的读取安全（虽然 zsim 的统计转储通常是序列化的）。

#### 数据结构提示 (Structural Hints)

构造函数中的 isRegular 参数是一个对后端的元数据提示 (Metadata Hint)。这允许结构化后端（如 HDF5）将这些数据扁平化存储为紧凑的多维数组（Array/Table），而不是创建数千个独立的 Group，从而显著提升存储效率和分析速度。

- Irregular (默认)：子节点结构各异。后端通常将其处理为树的遍历。
- Regular：表示该 Aggregate 的所有子节点都是同构的（Homogeneous）。例如，ProcStats 中包含 64 个结构完全相同的 Core 统计。

### PortVirtualizer

`PortVirtualizer` 是 zsim 实现的一个**用户态网络地址转换 (User-Space NAT)** 层。

它的主要目的是将 **Guest Application**（被模拟程序）看到的 **Virtual Port** 与 **Host OS**（宿主操作系统）分配的 **Real Port** 解耦。这样做是为了解决两个问题：

1. **端口冲突**：避免多个模拟进程同时绑定同一个常用端口（如 80）时，在 Host 上报 "Address already in use" 错误。
2. **确定性**：让模拟程序总是认为自己绑定到了它想要的端口，而不受 Host 实际网络环境的影响。

#### Data Structures

`PortVirtualizer` 维护了两个双向映射表，并使用 `futex` 锁保证线程安全：

- `realToVirt`: 映射 **Real Port** (Host 分配的端口) $\rightarrow$ **Virtual Port** (Guest 想要的端口)。
- `virtToReal`: 映射 **Virtual Port** $\rightarrow$ **Real Port**。
- `pvLock`: 保护上述映射表的 Lock，防止多线程竞争。

它通过拦截三个关键的 Socket 系统调用来实现虚拟化：`bind`, `getsockname`, 和 `connect`。

#### 拦截 `bind()` (`PatchBind`)

当 Guest 尝试绑定一个端口（例如 Virtual Port 8080）：

1. **Pre-Call (调用前)**:
      - zsim 拦截 `bind` 调用。
      - 获取 Guest 想要绑定的 Virtual Port (8080)。
      - **篡改参数**：将传给内核的端口号修改为 **0**。在 Socket 编程中，绑定端口 0 意味着让 OS 自动分配一个可用的 **Ephemeral Port**（临时端口）。
2. **Post-Call (调用后)**:
      - Host OS 返回成功，并实际分配了一个 Real Port（假设是 45123）。
      - zsim 调用 `getsockname` 获取这个 Real Port (45123)。
      - **注册映射**：调用 `registerBind(8080, 45123)`，将两者关联起来。
      - **恢复参数**：将内存中的参数改回 8080，让 Guest 以为它成功绑定了 8080。

#### 拦截 `getsockname()` (`PatchGetsockname`)

当 Guest 询问 "我绑定了哪个端口？"：

1. **Post-Call**:
      - Host OS 返回 Real Port (45123)。
      - zsim 拦截返回值。
      - **查表**：调用 `lookupVirt(45123)` 找到对应的 Virtual Port (8080)。
      - **覆盖**：将返回结构体中的端口号修改为 8080。Guest 看到的就是它最初请求的端口。

#### 拦截 `connect()` (`PatchConnect`)

当 Guest Client 尝试连接一个 Server：

1. **Pre-Call**:
      - Guest 试图连接 Virtual Port (8080)。
      - **查表**：zsim 调用 `lookupReal(8080)` 查找对应的 Real Port。
      - **篡改参数**：如果找到了记录（说明这个端口是另一个 zsim 线程/进程绑定的），就将连接目标修改为 Real Port (45123)。
      - 这样，连接请求就能正确地路由到 Host 上实际监听的 Socket。
...
这表明 zsim 支持 **Port Domains**。不同的 Domain 拥有独立的 `PortVirtualizer` 实例和映射表。这意味着处于不同 Domain 的进程可以各自绑定 Virtual Port 80 而互不干扰（它们会在 Host 上映射到不同的 Real Port），从而实现了网络命名空间的隔离。
